...
void apply_boundary() {
	...
	for (int j = 1; j < y+1; j++) {
		MPI_Sendrecv(&(cells[x][j].part_ids), 2*num_part_per_dim*num_part_per_dim, MPI_INT, east_rank, 1, &(cells[0][j].part_ids), 2*num_part_per_dim*num_part_per_dim, MPI_INT, west_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	...
	// MPI_Sendrecv(&(cells[0][sizei].part_ids), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, north_rank, 1, &(cells[0][0].part_ids), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, south_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	// MPI_Sendrecv(&(cells[0][sizei].count), (sizei+2), MPI_INT, north_rank, 1, &(cells[0][0].count), (sizei+2), MPI_INT, south_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	...
	// MPI_Sendrecv(&(cells[0][1].part_ids), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, south_rank, 1, &(cells[0][sizei+1].part_ids), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, north_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	// MPI_Sendrecv(&(cells[0][1].count), (sizei+2), MPI_INT, south_rank, 1, &(cells[0][sizei+1].count), (sizei+2), MPI_INT, north_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	...
	// MPI_Sendrecv(&(cell_part_ids_flat[sizej*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (1*2*num_part_per_dim*num_part_per_dim) + 0]), sizej*2*num_part_per_dim*num_part_per_dim, MPI_INT, east_rank, 0, &(cell_part_ids_flat[0*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (1*2*num_part_per_dim*num_part_per_dim) + 0]), sizej*2*num_part_per_dim*num_part_per_dim, MPI_INT, west_rank, 0, cart_comm, MPI_STATUS_IGNORE);
	// MPI_Sendrecv(&(cell_count_flat[sizej*(sizej+2) + 1]), sizej, MPI_INT, east_rank, 0, &(cell_count_flat[0*(sizej+2) + 1]), sizej, MPI_INT, west_rank, 0, cart_comm, MPI_STATUS_IGNORE);
	// MPI_Sendrecv(&(cells[0][1]).size, 1, MPI_INT, west_rank, 0, &(cells[0][sizej+1]).size, 1, MPI_INT, east_rank, 0, cart_comm, MPI_STATUS_IGNORE);

	// exchange right-most regular column with left-most ghost column 
	// MPI_Sendrecv(&(cell_part_ids_flat[1*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (1*2*num_part_per_dim*num_part_per_dim) + 0]), sizej*2*num_part_per_dim*num_part_per_dim, MPI_INT, west_rank, 0, &(cell_part_ids_flat[(sizej+1)*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (1*2*num_part_per_dim*num_part_per_dim) + 0]), sizej*2*num_part_per_dim*num_part_per_dim, MPI_INT, east_rank, 0, cart_comm, MPI_STATUS_IGNORE);
	...
	// smemcpy(particles.ay, temp_part_ay, num_particles_per_proc*sizeof(double));
