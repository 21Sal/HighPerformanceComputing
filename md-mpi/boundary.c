...
void apply_boundary() {
	...
	MPI_Sendrecv(&(cell_part_ids_flat[0*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (1*2*num_part_per_dim*num_part_per_dim) + 0]), (sizej+2)*2*num_part_per_dim*num_part_per_dim, MPI_INT, west_rank, 0, &(cell_part_ids_flat[0*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + ((sizej-1)*2*num_part_per_dim*num_part_per_dim) + 0]), (sizej+2)*2*num_part_per_dim*num_part_per_dim, MPI_INT, east_rank, 0, cart_comm, &status);
	...
	MPI_Sendrecv(&(cell_part_ids_flat[0*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (sizej*2*num_part_per_dim*num_part_per_dim) + 0]), (sizej+2)*2*num_part_per_dim*num_part_per_dim, MPI_INT, west_rank, 0, &(cell_part_ids_flat[0*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (0*2*num_part_per_dim*num_part_per_dim) + 0]), (sizej+2)*2*num_part_per_dim*num_part_per_dim, MPI_INT, east_rank, 0, cart_comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(cells[0][sizej].count), 1, mpi_count_column, west_rank, 0,&(cells[0][0].count), 1, mpi_count_column, east_rank, 0, cart_comm, MPI_STATUS_IGNORE);
	// MPI_Sendrecv(&(cells[0][sizej]).size, 1, MPI_INT, west_rank, 0, &(cells[0][0]).size, 1, MPI_INT, east_rank, 0, cart_comm, MPI_STATUS_IGNORE);

	MPI_Sendrecv(&(cell_part_ids_flat[1*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (0*2*num_part_per_dim*num_part_per_dim) + 0]), 1, mpi_part_ids_column, north_rank, 1, &(cell_part_ids_flat[(sizej+1)*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (0*2*num_part_per_dim*num_part_per_dim) + 0]), 1, mpi_part_ids_column, south_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	...
	MPI_Sendrecv(&(cell_part_ids_flat[sizei*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (1*2*num_part_per_dim*num_part_per_dim) + 0]), 1, mpi_part_ids_column, north_rank, 1, &(cell_part_ids_flat[1*(2*num_part_per_dim*num_part_per_dim)*(sizej+2) + (0*2*num_part_per_dim*num_part_per_dim) + 0]), 1, mpi_part_ids_column, south_rank, 1, cart_comm, MPI_STATUS_IGNORE);
