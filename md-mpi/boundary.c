...
void apply_boundary() {
	...
	MPI_Sendrecv(&(cell_count_flat[0*(sizej+2) + 1]), sizej, MPI_INT, west_rank, 0, &(cell_count_flat[0*(sizej+2) + sizej]), sizej, MPI_INT, east_rank, 0, cart_comm, &status);
	...
	MPI_Sendrecv(&(cell_count_flat[(sizej+1)*(sizej+2) + 1]), sizej, MPI_INT, west_rank, 0,&(cell_count_flat[1*(sizej+2) + 1]), sizej, MPI_INT, east_rank, 0, cart_comm, MPI_STATUS_IGNORE);
	...
	MPI_Sendrecv(&(cells.part_ids[0][0]), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, north_rank, 1, &(cells.part_ids[0][sizei]), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, south_rank, 1, cart_comm, MPI_STATUS_IGNORE);
	...
	MPI_Sendrecv(&(cells.part_ids[0][sizei+1]), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, north_rank, 1, &(cells.part_ids[0][sizei+1]), (sizei+2) * 2 * num_part_per_dim * num_part_per_dim, MPI_INT, south_rank, 1, cart_comm, MPI_STATUS_IGNORE);
